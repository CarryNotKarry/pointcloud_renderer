...
const ROI_LINE_RATIO = 4 / 480; // Shared convention with workbench_core.py.
...
function paint(draft=null) {
  for(const canvas of $('panels').querySelectorAll('canvas')){
    ...
    [...obj().rois,...(draft?[draft]:[])].forEach((r,i)=>{ctx.strokeStyle=colors[i%4];ctx.lineWidth=canvas.width*ROI_LINE_RATIO;ctx.strokeRect(r.x*480,r.y*480,r.w*480,r.h*480);ctx.fillStyle=colors[i%4];ctx.font='13px sans-serif';ctx.fillText('R'+(i+1),r.x*480+4,r.y*480+15);});
  }
}
...
function paintLive(camera,scale=state.style.size_scale){
  ...
  for(const canvas of $('panels').querySelectorAll('canvas')){
    ...
    const ctx=canvas.getContext('2d');obj().rois.forEach((r,i)=>{ctx.strokeStyle=colors[i];ctx.lineWidth=canvas.width*ROI_LINE_RATIO;ctx.strokeRect(r.x*480,r.y*480,r.w*480,r.h*480);});
  }
...
function roiList(){
  ...
  obj().rois.forEach((r,i)=>{
    ...
    const crops=element('div',undefined,'crops');for(const p of images){if(!p.img)continue;const box=element('div',undefined,'crop'),c=document.createElement('canvas');
      ...
      const w=Math.max(1,right-left),h=Math.max(1,bottom-top),line=480*ROI_LINE_RATIO,pad=Math.ceil(line/2)+1;
      c.width=w+2*pad;c.height=h+2*pad;const ctx=c.getContext('2d');ctx.fillStyle=state.style.background;ctx.fillRect(0,0,c.width,c.height);ctx.drawImage(p.img,left,top,w,h,pad,pad,w,h);
      ctx.strokeStyle=colors[i];ctx.lineWidth=line;ctx.strokeRect(pad,pad,w,h);
      const img=new Image();img.src=c.toDataURL();img.style.border='none';img.style.height='auto';box.append(img,element('small',p.label));crops.append(box);}
    $('rois').append(crops);
  });
}
